<template>
  <div class="table-container">
    ...
    >
      ...
      ></el-table-column>
      <el-table-column
        :formatter="identityValueFormatter"
        show-overflow-tooltip
        prop="identity"
        label="身份"
      ></el-table-column>
...
<script>
  ...
  import { getPlayerDetailList, getPlayerDetailListById, deletePlayerInfo} from '@/api/PlayerManagement'
  export default {
    ...
    methods: {
      ...
      identityValueFormatter(row, column) {
        if (row.identity == 0) {
          return '默认'
      ...
      dataFormat(queryForm) {
        ...
        if(isNaN(queryFormTemp.keyword)) {
          ...
          queryFormTemp["agentName"] = queryForm.keyword;
        }else{
          console.log('is a number')
          queryFormTemp["agentPid"] = queryForm.keyword;
        }
      ...
      async fetchData(queryForm) {
        ...
        var data = await getPlayerDetailList(queryFormTemp);
      ...
      async fetchDataByAgentName() {
        this.listLoading = true
        let data = await getPlayerDetailListById(this.queryForm.keyword)
        var a = []
        a = data.data
        this.list = a
